...
#### 백준풀기(7주차)
  ...
  - [boj2098](./4_7주차/boj2098.cpp) // 7-A, TSP(tsp, 외판원)
  ...
  - [boj1480](./4_7주차/boj1480.cpp) // 7-X
    - 오래 걸려서 혼자 풀었음

  - [boj3687] // 7-Y
    - 혼자 풀기 [실패](./4_7주차/boj3687_fail.cpp)하였음. 테케는 맞는데 시간초과 DP로 구성하는것에서 막힘
    - 강의
      1. 최대값을 구하는 방식은 그리디로 풀 수 있는 문제였음
      2. 짝수라면 `1`을 최대로 붙이는 게 최대 값, 홀수라면 `7`를 맨 앞자리로 해서 `1`을 최대한 붙이는 게 최대 값
      3. 최소값을 구하는 방식은 DP로 푸는 문제 였음
      4. 바텀업 또는 탑바텀으로 풀 수 있었음
        - [바텀업풀이](./4_7주차/boj3687_teacher_bottomUp.cpp)
        - [탑바텀풀이](./4_7주차/boj3687_teacher_topBottom.cpp)


### 2-12. 히든퀘스트_3

  - [boj24416](./4_히든퀘스트3/boj24416.cpp) // 1
    - 브론즈1, 혼자풀었음

  - [boj9184](./4_히든퀘스트3/boj9184.cpp) // 2
    - 실버2, 혼자풀었음

  - [boj1904](./4_히든퀘스트3/boj1904.cpp) // 3
    - 실버3, 혼자풀었음
    - 왜맞틀에서 약간 허우적됨.. 자세히 보니, `15746`으로 나눈 나머지를 출력하라는 조건을 안해줘서 틀린거였음

  - [boj9461](./4_히든퀘스트3/boj9461.cpp) // 4
    - 실버3, 혼자풀었음

  - [boj1912](./4_히든퀘스트3/boj1912.cpp) // 5
    - 실버2, 혼자풀었음
  
  - [boj1149](./4_히든퀘스트3/boj1149.cpp) // 6
    - 실버1, 혼자풀었음

  - [boj1932](./4_히든퀘스트3/boj1932.cpp) // 7
    - 실버1, 혼자풀었음

  - [boj2579](./4_히든퀘스트3/boj2579.cpp) // 8
    - 실버3, 혼자풀었음
    - 근데 어렵게 풀었다. DP 1차원 배열로 풀 수 있는문제임
    - [1차원DP풀이](./4_히든퀘스트3/boj2579_easily.cpp)
  ...
  - [boj10844] // 10
    - 실버1, 혼자풀었음
    - 점화식 어줍짢게 유도하려다가 일반화 실패해서... 완탐+메모이제이션 방식으로 풀긴했음
    - 점화식은 아래와 같이 유도할 수 있는 문제였음
    ```c++
    for(int i = 1; i < 10 ; ++i)
        DP[1][i] = 1;
    for(int i=2; i<=n ; ++i){
  ...
  - [boj11053] // 12
    - 실버2
    - [혼자 풀었음](./4_히든퀘스트3/boj11053.cpp)
    - 가장 긴 수열의 길이만 구하면 될 경우에는 이분탐색을 이용해서 구할 수 있는 문제임
      - [강의코드](./4_히든퀘스트3/boj11503_teacher.cpp)

  - [boj11054] // 13
    - 골드4
    - 혼자 풀었음, 질문 게시판의 반례를 참고하고 난 뒤 ㅠㅠ

  - 퀵정렬 스터디: `24.4/27`
    - [boj2751](./0_quickSort/qsort_2751.cpp)
      - 피봇 자체도 랜덤으로 지정해야지만 테스트 통과 가능함
      - 퀵정렬이라는 것이 악의조건에서는 O(N^2)인 로직임
      - 퀵정렬을 다시 이해해야하는 순간이 오면 아래의 반례를 손으로 정리해볼것
        - `반례: 7,6,5,1,2,3,4,9`
      - 핵심 코드
        ![](img/2024-04-27-17-46-47.png)
    - [boj11650](./0_quickSort/qsort_11650.cpp)
    - [boj11651](./0_quickSort/qsort_11651.cpp)
        - key 2개로 하는 퀵 정렬
    - [qSortPair](./0_quickSort/qsort_pair.cpp)
      - 첫번째 요소는 내림차순 정렬
      - 첫번째 요소가 같다면 두번째 요소의 오름차순 정렬
...
### 2-13. 8주차 펜윅트리 최단거리
  - [블로그자료](https://blog.naver.com/jhc9639/222349335717)  

  #### 1. 펜윅트리
  - 펜윅트리는 이진트리 기반의 자료구조이며 세그먼트 트리의 원리를 가지고 있음
    - ![](img/2024-05-06-22-26-58.png)
    - 최하위노드 `(idx&-idx)`를 빼가며 갱신하고 업데이트하는 간단한 트리, Binary Indexed Tree라고도 함. 세그먼트 트리와는 다르게 모든 세그먼트를 만들 필요가 없음
  ...
  - 펜윅트리 개념문제
    1. 펜윅트리를 활용한 문제 (구간합)
      - [boj2042](./5_8주차/boj2042.cpp)
    2. 펜윅트리 문제 (카운팅 트리)  
      - [boj3653](./5_8주차/boj3643.cpp)

  #### 2. 최단거리
  - 최단거리 알고리즘 (트리)
  - `정점에서 정점까지의 최단거리를 알고 싶어`
  ![](img/2024-05-07-00-21-49.png)

    1. 다익스트라(from-to)
      - 시작점이 정해져 있는 거리 배열 트리
      - ![](img/2024-05-06-23-35-45.png)
      - 시간복잡도(`O(E logV)`)
      - 모든 정점간 이동거리에 대한 최소(대)값을 갱신하는 트리 알고리즘
      - 모든 세그먼트에 대한 최소(대)값을 지닌 트리를 완성할 수 있다.
      - 거리값을 알 수 있다.
      
    2. 플로이드워셜
      - 모든 쌍에 대해!
      - ![](img/2024-05-07-00-17-07.png)
      - 시간복잡도(`O(V^3)`)
      - 3중 for-loop
  ...
  - 다익스트라, 플로이드워셜은 자주 나옴, 벨만포드도 알면 좋다.
 ...
 #### 백준풀기 (8주차)
  ...
  - [boj5419] // 8-D, 어려운문제 `TOP3`
    - 혼자 못 풀었음
      - 펜웍트리를 사용한 문제인데,, 어렵다 ㅠㅠ
    - 펜윅트리는 최하위 노드부터 더하거나,빼는 방식으로 트리를 업데이트하는 자료구조임(복습)
      - 코드 동작 살펴보았고, 인강부터 듣기
    - 강의
      1. Y축 대칭 정렬
      2. 카운팅배열
  ...
  - [boj2302](./5_8주차/boj2302.cpp) // 8-E
    - 혼자 풀긴했음, (실버1)
      - 실버1인데, 오랜만에 풀어서 끙끙.. ㅠㅠ
