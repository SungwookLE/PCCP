...
{   
    ...
    int PivotCompare1 = V[mid][1];
    int PivotCompare2 = V[mid][0];

    while (i <= j)
    {

        while ( V[i][1] < PivotCompare1 || ((V[i][1] == PivotCompare1) && (V[i][0] < PivotCompare2)) ){
            i++;
        }

        while ( PivotCompare1 < V[j][1] || ((V[j][1] == PivotCompare1) && (PivotCompare2 < V[j][0])) ) 
        {
            j--;
        }
